...
/*   Updated: 2023/11/23 07:58:01 by ealislam         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (remain[fd])
	{
		if (append_to_final_until_n(remain[fd], &fnl_str) || \
			trim_remain(&(remain[fd])))
			return (free_and_return_null(&(remain[fd]), &fnl_str, &cur_buf));
		if (check_n(fnl_str))
			return (fnl_str);
	}
	if (read_file(fd, &cur_buf, &fnl_str) || \
		get_remain(&(remain[fd]), cur_buf) || \
		(fnl_str && !*fnl_str))
		return (free_and_return_null(&(remain[fd]), &fnl_str, &cur_buf));
	if (cur_buf)
		free (cur_buf);
